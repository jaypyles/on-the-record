...
export const ShopPage = ({ items }: ShopPageProps) => {
  const router = useRouter();
  const { type, price } = router.query;
  ...
  useEffect(() => {
    ...
    return () => clearTimeout(handler);
  }, [selectedType, debouncedSearch, priceRange]);
  ...
  const { data, isLoading } = trpc.artistRouter.get.useQuery(
    ...
    {
      placeholderData: (previousData) => previousData || items,
                ...
                <div className="space-y-2">
                  ...
                  <label className="flex items-center">
                    <input
                      ...
                      onChange={() => setSelectedType(ShopItemType.CD)}
                      className="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300"
                    />
                  ...
                  <label className="flex items-center">
                    <input
                      ...
                      onChange={() => setSelectedType(ShopItemType.VINYL)}
                      className="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300"
                    />
                  ...
                  <label className="flex items-center">
                    <input
                      ...
                      onChange={() => setSelectedType(ShopItemType.MERCH)}
                      className="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300"
                    />
                    <span className="ml-2 text-sm text-gray-700">
                      Merchandise
